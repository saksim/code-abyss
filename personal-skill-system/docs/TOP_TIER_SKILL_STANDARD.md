...
- reference placeholders that match the layer's top-tier depth floor
  domain and workflow scaffolds should start with at least three reference placeholders; router, tool, and guard scaffolds should start with at least two
...
- for new domain/workflow skills, optionally scaffold capability governance surfaces at creation time so module-groups, route expert-modules, and initial thin ratings do not start as manual debt
