...
| Skills Source | `personal-skill-system/skills/` | 权威 skill source tree（构建与分发来源） |
...
| Codex | `~/.codex/config.toml`、`instruction.md`、`AGENTS.md`、`~/.agents/skills/` | Codex 走 skills-first 运行形态，`~/.agents/skills/` 为主入口；`~/.codex/skills/` 仅兼容镜像 |
...
personal-skill-system/skills/ 权威 skills source tree
skills/              已退役（root mirror 已移除，禁止回灌）
...
- 这个仓库已经从 legacy Codex prompts 形态迁移出来，维护时应以 `config.toml`、`instruction.md`、`AGENTS.md`、`personal-skill-system/skills/`（source）与 `~/.agents/skills/`（runtime）为准；root `skills/` 已退役并移除。
