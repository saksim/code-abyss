...
function getGeneratedWriteRequirements(projectRoot) {
  ...
  return [
    ...
    {
      path: getEvolutionLedgerPath(projectRoot),
      mode: 'rewrite-file',
      label: 'evolution ledger registry'
    },
    {
      path: getReviewQueueRegistryPath(projectRoot),
      mode: 'rewrite-file',
      label: 'review queue registry'
    },
    {
      path: getSkillOpportunityQueueRegistryPath(projectRoot),
      mode: 'rewrite-file',
      label: 'skill opportunity queue registry'
    },
    {
      path: getSkillInvestmentBacklogRegistryPath(projectRoot),
      mode: 'rewrite-file',
    ...
    {
      path: path.join(bundleRoot, 'benchmark', 'host-smoke', 'runtime-runs'),
      mode: 'write-dir',
...
function buildCapabilityModuleScaffolds(projectRoot, kind, skillName, skillText, targetDir) {
  ...
  return referencePaths.map((refPath) => {
    ...
    return {
      id: `${skillName}-${slug}`,
      path: toPortablePath(getBundleRoot(projectRoot), absoluteRefPath),
...
function showSkillInvestmentBacklog(projectRoot, options = {}) {
  const backlogPath = getSkillInvestmentBacklogRegistryPath(projectRoot);
  const payload = fs.existsSync(backlogPath)
    ? readJson(backlogPath)
    : buildSkillInvestmentBacklog(getBundleRoot(projectRoot), {
...
function main(argv) {
  ...
  if (!action) {
    fail('usage: manage-skill <record-opportunity|show-opportunity-queue|resolve-opportunity|admission-check|show-admission-ledger|resolve-admission|evolution-check|show-evolution-ledger|resolve-evolution|show-review-queue|show-investment-backlog|mark-reviewed|assess-top-tier|create|show|update|set-status|set-module-rating|archive|delete|sync-scaffold-lineage|sync-runtime-proof|sync-host-metadata|sync-route-metadata|run-host-smoke|reconcile-host-smoke> ...');
  }
  ...
  if (action === 'create') {
    ...
    return createSkill(arg1, arg2, { scaffoldModules, requestId, opportunityId });
  }
