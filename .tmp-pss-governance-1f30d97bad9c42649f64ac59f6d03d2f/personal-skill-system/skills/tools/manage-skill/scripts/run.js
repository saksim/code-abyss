...
function syncRatingsOnCreate(projectRoot, skillName) {
  ...
  summary['top-level-enough-now'] = summary['top-level-enough-now'].filter((name) => name !== skillName);
  summary['useful-overlay-not-top-level-alone'] = summary['useful-overlay-not-top-level-alone'].filter((name) => name !== skillName);
  if (!summary['strong-uplift-but-not-top-yet'].includes(skillName)) {
    summary['strong-uplift-but-not-top-yet'].push(skillName);
    summary['strong-uplift-but-not-top-yet'].sort();
  }
...
function syncRatingsOnRemove(projectRoot, skillName) {
  ...
  const ratings = readJson(ratingsPath);
  const summary = ratings['skill-level-summary'] || {};
  summary['top-level-enough-now'] = removeFromArray(summary['top-level-enough-now'], skillName);
  summary['strong-uplift-but-not-top-yet'] = removeFromArray(summary['strong-uplift-but-not-top-yet'], skillName);
  summary['useful-overlay-not-top-level-alone'] = removeFromArray(summary['useful-overlay-not-top-level-alone'], skillName);
  ratings['skill-level-summary'] = summary;
  recomputeSkillLevelSummary(ratings);
  writeJson(ratingsPath, ratings);
  syncCapabilityRatingsDoc(projectRoot);
...
function archiveSkill(skillName) {
  ...
  const next = renderSkillFile(resolved.parsed);
  fs.writeFileSync(resolved.skillFile, next, 'utf8');
  syncArchiveOnGeneratedSurfaces(getProjectRoot(), skillName);
...
function main(argv) {
  ...
  if (!action) {
    fail('usage: manage-skill <create|show|update|archive|delete|sync-runtime-proof|run-host-smoke> ...');
  }
